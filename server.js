...
app.get("/", (req, res) => {
  ...
  fs.readFile("EmpDetails.txt", (err, data) => {
    ...
    arrayOfStrings.map(
      (emp, index) =>
        (body += `<tr><td >${index + 1}</td><td >${emp.name}</td><td>${
          emp.email
        }</td><td>${emp.age}</td><td>${emp.city}</td><td>${
          emp.salary
        }</td><td><a href="/delete/${index}" class="btn btn-danger">Delete</a> <a href="/update/${index}" class="btn btn-danger">Update</a></td></tr>`)
    );
    if (err) throw err;
...
app.get("/update/:id", function (req, res) {
  ...
  fs.readFile("Empdetails.txt", (err, data) => {
    ...
    app.post("/update_data/:num", (req, res) => {
      let num = req.params.num;
      console.log("nummmmmm", num)
      console.log(id, "id", details)
      details[num].name = req.body.name;
      details[num].email = req.body.email;
      details[num].age = req.body.age;
      details[num].city = req.body.city;
      details[num].salary = req.body.salary;
      data = details;
